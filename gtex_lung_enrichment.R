library(readr)
library(dplyr)
library(regioneR)
library(ggplot2)
library(tidyr)

set.seed(252)
thousand_genome_panel_hg38 = read_tsv("/Users/ashvinravi/Desktop/bmi206/individual_project/thousand_genome_panel_hg38.tsv.gz")
# copd_gwas = read_tsv("/Users/ashvinravi/Desktop/bmi206/individual_project/copd_gwas_summary_stats_hg19.tsv.bgz")
# 
# copd_gwas <- copd_gwas %>% distinct(variant, .keep_all = T)
# significant_snps <- copd_gwas %>% filter(pval <= (0.05 / nrow(copd_gwas)))
# 
# significant_snps <- significant_snps %>% separate(variant, into = c('chrom', 'pos', "A1", "A2"), sep = ":")
# significant_snps$chrom <- paste0("chr", significant_snps$chrom)
# significant_snps$SNP <- paste0("rs", 1:nrow(significant_snps_coords_hg19))
# 
# significant_snps_coords_hg19 <- significant_snps %>% select(chrom, pos, SNP) %>% mutate(end=as.numeric(pos) + 1) 
# significant_snps_coords_hg19 <- significant_snps_coords_hg19[, c(1,2,4,3)]
# significant_snps_coords_hg19$SNP <- paste0("rs", 1:nrow(significant_snps_coords_hg19))
# write.table(significant_snps_coords_hg19, "/Users/ashvinravi/Desktop/bmi206/individual_project/significant_snps_copd_coords_hg19.bed", sep="\t", quote=F, col.names=F, row.names=F)
# 
# significant_snps_coords_hg38 <- read.table("/Users/ashvinravi/Desktop/bmi206/individual_project/significant_snps_copd_coords_hg38.bed", sep="\t")
# colnames(significant_snps_coords_hg38) = c("chrom", "start_hg38", "end_hg38", "SNP")
# significant_snps_hg38 <- inner_join(significant_snps_coords_hg38, significant_snps, by = c("chrom", "SNP"))
# 
# significant_snps_hg38 <- significant_snps_hg38 %>% select(-c(pos, SNP))
# names(significant_snps_hg38)[names(significant_snps_hg38) == "start_hg38"] = "start"
# names(significant_snps_hg38)[names(significant_snps_hg38) == "end_hg38"] = "end"

gwas_catalog <- read_tsv("/Users/ashvinravi/Desktop/bmi206/group_project/gwas_catalog_v1.0-associations_e110_r2023-11-08.tsv")
european_gwas_catalog <- gwas_catalog %>% 
  filter(grepl("European", `INITIAL SAMPLE SIZE`)) %>% 
  filter(grepl("Lung", `DISEASE/TRAIT`)) %>% 
  select(CHR_ID, CHR_POS, SNPS) 

gwas_snps <- european_gwas_catalog %>% distinct(CHR_ID, CHR_POS, .keep_all = T)
gwas_snps = gwas_snps[!grepl("x", gwas_snps$CHR_POS),]
gwas_snps = gwas_snps[!grepl(";", gwas_snps$CHR_POS),]
gwas_snps = gwas_snps[!is.na(gwas_snps$CHR_POS),]
colnames(gwas_snps) = c("chrom", "start", "SNP")

gwas_snps$start = as.numeric(gwas_snps$start)
gwas_snps$end = gwas_snps$start + 1 
gwas_snps$chrom = paste0("chr", gwas_snps$chrom)
gwas_snps_maf <- inner_join(gwas_snps, thousand_genome_panel_hg38, by = c("chrom", "start", "end", "SNP"))

dist.fun.gwas = approxfun(density.default(gwas_snps_maf$MAF))

###### LUNG UNION SAMPLES 

lung_dhs_wout_footprints = read.table("/Users/ashvinravi/Desktop/bmi206/individual_project/lung_footprint_data/lung_union_footprints/dhs_peaks_wout_footprints.bed", sep='\t')
lung_0.001 = read.table("/Users/ashvinravi/Desktop/bmi206/individual_project/lung_footprint_data/lung_union_footprints/interval.all.fps.0.001.bed")
lung_0.01 = read.table("/Users/ashvinravi/Desktop/bmi206/individual_project/lung_footprint_data/lung_union_footprints/interval.all.fps.0.01.bed")
lung_0.05 = read.table("/Users/ashvinravi/Desktop/bmi206/individual_project/lung_footprint_data/lung_union_footprints/interval.all.fps.0.05.bed")

colnames(lung_dhs_wout_footprints) = c("chr", "start", "end")
colnames(lung_0.001) = c("chr", "start", "end")
colnames(lung_0.01) = c("chr", "start", "end")
colnames(lung_0.05) = c("chr", "start", "end")


dhs_wout_footprints_GR = toGRanges(data.frame(chr=lung_dhs_wout_footprints$chr,
                                              start=lung_dhs_wout_footprints$start, 
                                              end=lung_dhs_wout_footprints$end))

consensus_0.001_GR = toGRanges(data.frame(chr=lung_0.001$chr, 
                                          start=lung_0.001$start, 
                                          end=lung_0.001$end))
consensus_0.01_GR = toGRanges(data.frame(chr=lung_0.01$chr, 
                                         start=lung_0.01$start, 
                                         end=lung_0.01$end)) 
consensus_0.05_GR = toGRanges(data.frame(chr=lung_0.05$chr, 
                                         start=lung_0.05$start, 
                                         end=lung_0.05$end)) 

gwas_GR = toGRanges(data.frame(chr=gwas_snps_maf$chrom, 
                               start=gwas_snps_maf$start,
                               end=(gwas_snps_maf$end)))

gwas_overlap_dhs = numOverlaps(dhs_wout_footprints_GR, gwas_GR)
gwas_overlap_0.05 = numOverlaps(consensus_0.05_GR, gwas_GR)
gwas_overlap_0.01 = numOverlaps(consensus_0.01_GR, gwas_GR)
gwas_overlap_0.001 = numOverlaps(consensus_0.001_GR, gwas_GR)

randomized_overlap_0.05 = c()
randomized_overlap_0.01 = c()
randomized_overlap_0.001 = c()
randomized_overlap_dhs = c()

print("Generating random regions...")
for (x in 1:1000) {
  # Sample Random gwas_snps from panel from 1000G panel. 
  randomized_gwas_snps <- thousand_genome_panel_hg38 %>%
    slice_sample(n = 2673,
                 weight_by = dist.fun.gwas(MAF),
                 replace=T) %>%
    ungroup()
  print(x)
  randomized_gwas_snps_GR = toGRanges(data.frame(chr=randomized_gwas_snps$chrom,
                                                 start=randomized_gwas_snps$start,
                                                 end=as.numeric(randomized_gwas_snps$end)))
  
  randomized_overlap_dhs = c(randomized_overlap_dhs, numOverlaps(randomized_gwas_snps_GR, dhs_wout_footprints_GR) + 1)
  randomized_overlap_0.05 = c(randomized_overlap_0.05, numOverlaps(randomized_gwas_snps_GR, consensus_0.05_GR) + 1)
  randomized_overlap_0.01 = c(randomized_overlap_0.01, numOverlaps(randomized_gwas_snps_GR, consensus_0.01_GR) + 1)
  randomized_overlap_0.001 = c(randomized_overlap_0.001, numOverlaps(randomized_gwas_snps_GR, consensus_0.001_GR) + 1)
}

enrichment_dhs_gwas = data.frame(gwas_overlap_dhs / randomized_overlap_dhs)
enrichment_dhs_gwas$category = "DHS w/out footprints"
colnames(enrichment_dhs_gwas) = c("enrichment_ratio", "Category")
enrichment_0.05_gwas = data.frame(gwas_overlap_0.05 / randomized_overlap_0.05)
enrichment_0.05_gwas$category = "0.05"
colnames(enrichment_0.05_gwas) = c("enrichment_ratio", "Category")
enrichment_0.01_gwas = data.frame(gwas_overlap_0.01 / randomized_overlap_0.01)
enrichment_0.01_gwas$category = "0.01"
colnames(enrichment_0.01_gwas) =  c("enrichment_ratio", "Category")
enrichment_0.001_gwas = data.frame(gwas_overlap_0.001 / randomized_overlap_0.001)
enrichment_0.001_gwas$category = "0.001"
colnames(enrichment_0.001_gwas) = c("enrichment_ratio", "Category")
enrichment_table_gwas = rbind(enrichment_dhs_gwas, enrichment_0.05_gwas, enrichment_0.01_gwas, enrichment_0.001_gwas)
enrichment_table_gwas$Category = factor(enrichment_table_gwas$Category, levels=c("DHS w/out footprints", "0.05", "0.01", "0.001"))
write_tsv(enrichment_table_gwas, "/Users/ashvinravi/Desktop/bmi206/individual_project/lung_gwas_enrichment_table.tsv")

boxplot <- ggplot(enrichment_table_gwas, aes(x=Category, y=enrichment_ratio, fill=Category)) + geom_boxplot() + theme_classic()

##################### GTEX eQTL LUNG ENRICHMENT 

gtex_lung_egenes <- read_tsv("/Users/ashvinravi/Desktop/bmi206/individual_project/GTEx_Analysis_v8_eQTL/Lung.v8.egenes.txt.gz")
significant_variant_pairs <- read_tsv("/Users/ashvinravi/Desktop/bmi206/individual_project/GTEx_Analysis_v8_eQTL/Lung.v8.signif_variant_gene_pairs.txt.gz")

significant_variant_pairs <- significant_variant_pairs %>% separate(variant_id, into = c("chrom", "pos", "A1", "A2", "genome_build"), sep = "_")
significant_variants <- significant_variant_pairs %>% select(chrom, pos) %>% mutate(end = as.numeric(pos) + 1)

lead_lung_eQTL_variants <- gtex_lung_egenes %>% 
  filter(qval <= 0.05) %>% 
  select(chr, variant_pos, rs_id_dbSNP151_GRCh38p7) %>% 
  distinct()

colnames(lead_lung_eQTL_variants) = c("chrom", "start", "SNP")
lead_lung_eQTL_variants <- inner_join(lead_lung_eQTL_variants, thousand_genome_panel_hg38, by = c("chrom", "start", "SNP"))

dist.fun.eQTL <- approxfun(density.default(lead_lung_eQTL_variants$MAF))

eQTL_GR = toGRanges(data.frame(chr=lead_lung_eQTL_variants$chrom, 
                               start=lead_lung_eQTL_variants$start,
                               end=as.numeric(lead_lung_eQTL_variants$start) + 1))

eQTL_overlap_dhs = numOverlaps(dhs_wout_footprints_GR, eQTL_GR)
eQTL_overlap_0.05 = numOverlaps(consensus_0.05_GR, eQTL_GR)
eQTL_overlap_0.01 = numOverlaps(consensus_0.01_GR, eQTL_GR)
eQTL_overlap_0.001 = numOverlaps(consensus_0.001_GR, eQTL_GR)

randomized_overlap_0.05 = c()
randomized_overlap_0.01 = c()
randomized_overlap_0.001 = c()
randomized_overlap_dhs = c()

print("Generating random regions...")
for (x in 1:1000) {
  # Sample Random gwas_snps from panel from 1000G panel. 
  randomized_eQTL_snps <- thousand_genome_panel_hg38 %>%
    slice_sample(n = 10921, 
                 weight_by = dist.fun.eQTL(MAF),
                 replace=T) %>%
    ungroup()
  print(x)
  randomized_eQTL_snps_GR = toGRanges(data.frame(chr=randomized_eQTL_snps$chrom,
                                                 start=randomized_eQTL_snps$start,
                                                 end=as.numeric(randomized_eQTL_snps$end)))
  
  randomized_overlap_dhs = c(randomized_overlap_dhs, numOverlaps(randomized_eQTL_snps_GR, dhs_wout_footprints_GR) + 1)
  randomized_overlap_0.05 = c(randomized_overlap_0.05, numOverlaps(randomized_eQTL_snps_GR, consensus_0.05_GR) + 1)
  randomized_overlap_0.01 = c(randomized_overlap_0.01, numOverlaps(randomized_eQTL_snps_GR, consensus_0.01_GR) + 1)
  randomized_overlap_0.001 = c(randomized_overlap_0.001, numOverlaps(randomized_eQTL_snps_GR, consensus_0.001_GR) + 1)
}

enrichment_dhs_eQTL = data.frame(eQTL_overlap_dhs / randomized_overlap_dhs)
enrichment_dhs_eQTL$category = "DHS w/out footprints"
colnames(enrichment_dhs_eQTL) = c("enrichment_ratio", "Category")
enrichment_0.05_eQTL = data.frame(eQTL_overlap_0.05 / randomized_overlap_0.05)
enrichment_0.05_eQTL$category = "0.05"
colnames(enrichment_0.05_eQTL) = c("enrichment_ratio", "Category")
enrichment_0.01_eQTL = data.frame(eQTL_overlap_0.01 / randomized_overlap_0.01)
enrichment_0.01_eQTL$category = "0.01"
colnames(enrichment_0.01_eQTL) =  c("enrichment_ratio", "Category")
enrichment_0.001_eQTL = data.frame(eQTL_overlap_0.001 / randomized_overlap_0.001)
enrichment_0.001_eQTL$category = "0.001"
colnames(enrichment_0.001_eQTL) = c("enrichment_ratio", "Category")
enrichment_table_eQTL = rbind(enrichment_dhs_eQTL, enrichment_0.05_eQTL, enrichment_0.01_eQTL, enrichment_0.001_eQTL)
enrichment_table_eQTL$Category = factor(enrichment_table_eQTL$Category, levels=c("DHS w/out footprints", "0.05", "0.01", "0.001"))
write_tsv(enrichment_table_eQTL, "/Users/ashvinravi/Desktop/bmi206/individual_project/lung_eQTL_enrichment_table.tsv")

boxplot_eQTL <- ggplot(enrichment_table_eQTL, aes(x=Category, y=enrichment_ratio, fill=Category)) + geom_boxplot() + theme_classic()

######### sQTL enrichment 
gtex_lung_sgenes <- read_tsv("/Users/ashvinravi/Desktop/bmi206/individual_project/GTEx_Analysis_v8_sQTL/Lung.v8.sgenes.txt.gz")
significant_splicing_variant_pairs <- read_tsv("/Users/ashvinravi/Desktop/bmi206/individual_project/GTEx_Analysis_v8_sQTL/Lung.v8.sqtl_signifpairs.txt.gz")

significant_splicing_variant_pairs <- significant_splicing_variant_pairs %>% separate(variant_id, into = c("chrom", "pos", "A1", "A2", "genome_build"), sep = "_")
significant_splicing_variants <- significant_splicing_variant_pairs %>% select(chrom, pos) %>% mutate(end = as.numeric(pos) + 1)

lead_lung_sQTL_variants <- gtex_lung_sgenes %>% 
  filter(qval <= 0.05) %>% 
  select(chr, variant_pos, rs_id_dbSNP151_GRCh38p7) %>% 
  distinct()

colnames(lead_lung_sQTL_variants) = c("chrom", "start", "SNP")
lead_lung_sQTL_variants <- inner_join(lead_lung_sQTL_variants, thousand_genome_panel_hg38, by = c("chrom", "start", "SNP"))

dist.fun.sQTL <- approxfun(density.default(lead_lung_sQTL_variants$MAF))

sQTL_GR = toGRanges(data.frame(chr=lead_lung_sQTL_variants$chrom, 
                               start=lead_lung_sQTL_variants$start,
                               end=as.numeric(lead_lung_sQTL_variants$start) + 1))

sQTL_overlap_dhs = numOverlaps(dhs_wout_footprints_GR, sQTL_GR)
sQTL_overlap_0.05 = numOverlaps(consensus_0.05_GR, sQTL_GR)
sQTL_overlap_0.01 = numOverlaps(consensus_0.01_GR, sQTL_GR)
sQTL_overlap_0.001 = numOverlaps(consensus_0.001_GR, sQTL_GR)

randomized_overlap_sQTL_0.05 = c()
randomized_overlap_sQTL_0.01 = c()
randomized_overlap_sQTL_0.001 = c()
randomized_overlap_sQTL_dhs = c()

print("Generating random regions...")
for (x in 1:1000) {
  # Sample Random gwas_snps from panel from 1000G panel. 
  randomized_sQTL_snps <- thousand_genome_panel_hg38 %>%
    slice_sample(n = 2846, 
                 weight_by = dist.fun.sQTL(MAF),
                 replace=T) %>%
    ungroup()
  print(x)
  randomized_sQTL_snps_GR = toGRanges(data.frame(chr=randomized_sQTL_snps$chrom,
                                                 start=randomized_sQTL_snps$start,
                                                 end=as.numeric(randomized_sQTL_snps$end)))
  
  randomized_overlap_sQTL_dhs = c(randomized_overlap_sQTL_dhs, numOverlaps(randomized_sQTL_snps_GR, dhs_wout_footprints_GR) + 1)
  randomized_overlap_sQTL_0.05 = c(randomized_overlap_sQTL_0.05, numOverlaps(randomized_sQTL_snps_GR, consensus_0.05_GR) + 1)
  randomized_overlap_sQTL_0.01 = c(randomized_overlap_sQTL_0.01, numOverlaps(randomized_sQTL_snps_GR, consensus_0.01_GR) + 1)
  randomized_overlap_sQTL_0.001 = c(randomized_overlap_sQTL_0.001, numOverlaps(randomized_sQTL_snps_GR, consensus_0.001_GR) + 1)
}

enrichment_dhs_sQTL = data.frame(sQTL_overlap_dhs / randomized_overlap_sQTL_dhs)
enrichment_dhs_sQTL$category = "DHS w/out footprints"
colnames(enrichment_dhs_sQTL) = c("enrichment_ratio", "Category")
enrichment_0.05_sQTL = data.frame(sQTL_overlap_0.05 / randomized_overlap_sQTL_0.05)
enrichment_0.05_sQTL$category = "0.05"
colnames(enrichment_0.05_sQTL) = c("enrichment_ratio", "Category")
enrichment_0.01_sQTL = data.frame(sQTL_overlap_0.01 / randomized_overlap_sQTL_0.01)
enrichment_0.01_sQTL$category = "0.01"
colnames(enrichment_0.01_sQTL) =  c("enrichment_ratio", "Category")
enrichment_0.001_sQTL = data.frame(sQTL_overlap_0.001 / randomized_overlap_sQTL_0.001)
enrichment_0.001_sQTL$category = "0.001"
colnames(enrichment_0.001_sQTL) = c("enrichment_ratio", "Category")
enrichment_table_sQTL = rbind(enrichment_dhs_sQTL, enrichment_0.05_sQTL, enrichment_0.01_sQTL, enrichment_0.001_sQTL)
enrichment_table_sQTL$Category = factor(enrichment_table_sQTL$Category, levels=c("DHS w/out footprints", "0.05", "0.01", "0.001"))
write_tsv(enrichment_table_sQTL, "/Users/ashvinravi/Desktop/bmi206/individual_project/lung_sQTL_enrichment_table.tsv")

boxplot_sQTL <- ggplot(enrichment_table_sQTL, aes(x=Category, y=enrichment_ratio, fill=Category)) + geom_boxplot() + theme_classic()


